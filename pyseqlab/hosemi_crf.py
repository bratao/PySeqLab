...
class HOSemiCRF(HOSemiCRFAD):
    ...
    def compute_backward_vec(self, w, seq_id):
        ...
        for j in reversed(range(1, T + 1)):
            accumulator = (
                numpy.ones((len(S_codebook), L), dtype="longdouble") * -numpy.inf
            )
            for d in range(L):
                u = j
                v = j + d
                ...
                for si in si_siy_codebook:
                    si_c = S_codebook[si]
                    vec = (
                        b_potential[si_siy_codebook[si][0]]
                        + beta[v + 1, si_siy_codebook[si][1]]
                    )
            ...
            for d in range(L):
                ...
                if boundary in fpotential_perboundary:
                    ...
                    for z in Z_codebook:
                        ...
                        numerator = (
                            alpha[u - 1, pi_c] + f_potential[piy_c] + beta[v + 1, piy_c]
                        )
