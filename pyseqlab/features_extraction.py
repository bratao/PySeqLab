...
class FeatureExtractor(object):
        ...
        if type(template) == dict:
            ...
            for attr_name, templateX in template.items():
                ...
                for offset_x, offsets_y in templateX.items():
                    ...
                    feature_name = "|".join(
                        [attr_name + "[" + str(ofst_x) + "]" for ofst_x in s_offset_x]
                    )
                    ...
                    if unique_dict:
                        self._template_X[attr_name][s_offset_x] = tuple(
                            unique_dict.keys()
                        )
        ...
        for attr_name in template_X:
            ...
            for offset_tup_x in template_X[attr_name]:
                ...
                if attributes:
                    ...
                    feat_template[offset_tup_x] = attr_represent_func(
                        attributes, x_featurenames[attr_name][offset_tup_x]
                    )
                    ...
                    if offset_tup_y in y_feat_template:
                        for y_patt in y_feat_template[offset_tup_y]:
                            if y_patt in xy_features:
                                xy_features[y_patt].update(
                                    seg_feat_template[offset_tup_x]
                                )
                            else:
                                xy_features[y_patt] = dict(
                                    seg_feat_template[offset_tup_x]
                                )
        ...
        for j in range(1, T + 1):
            for d in range(L):
                ...
                if learning:
                    ...
                    seg_features[boundary] = self.flatten_segfeatures(
                        l_segfeatures[boundary]
                    )
                else:
    ...
    def save(self, folder_dir):
        ...
        save_info = {"FE_templateX": self.template_X, "FE_templateY": self.template_Y}
...
class SeqsRepresenter(object):
    ...
    def prepare_seqs(
        self, seqs_dict, corpus_name, working_dir, unique_id=True, log_progress=True
    ):
        ...
        target_dir = create_directory(
            "global_features", create_directory(corpus_folder, working_dir)
        )
        seqs_info = {}
    ...
    def preprocess_attributes(self, seqs_id, seqs_info, method="rescaling"):
        ...
        if grouped_attr.get("continuous"):
            ...
            active_continuous_attr = [
                attr for attr in active_attr if attr in grouped_attr["continuous"]
            ]
        else:
            active_continuous_attr = {}
        ...
        for seq_id in seqs_id:
            ...
            if seq_L > 1 or active_continuous_attr:
                ...
                seq = ReaderWriter.read_data(
                    os.path.join(seq_dir, "sequence"), mode="rb"
                )
            ...
            if seq_L > 1:
                ...
                if new_boundaries:
                    ReaderWriter.dump_data(
                        seq, os.path.join(seq_dir, "sequence"), mode="wb"
                    )
    ...
    def extract_seqs_globalfeatures(
        self, seqs_id, seqs_info, dump_gfeat_perboundary=False
    ):
        ...
        for seq_id in seqs_id:
            ...
            gfeatures_perboundary = feature_extractor.extract_seq_features_perboundary(
                seq
            )
            y_boundaries = seq.y_sboundaries
            # gfeatures has this format {'Y_patt':Counter(features)}
            gfeatures = feature_extractor.aggregate_seq_features(
                gfeatures_perboundary, y_boundaries
            )
            ...
            if dump_gfeat_perboundary:
                ReaderWriter.dump_data(
                    gfeatures_perboundary,
                    os.path.join(seq_dir, "globalfeatures_per_boundary"),
                )
            counter += 1
    ...
    def extract_seqs_modelactivefeatures(
        self, seqs_id, seqs_info, model, output_foldername, learning=False
    ):
        ...
        output_dir = create_directory(
            "model_activefeatures_{}".format(output_foldername), root_dir
        )
        L = model.L
        ...
        for seq_id in seqs_id:
            ...
            seg_features, l_segfeatures = f_extractor.lookup_seq_modelactivefeatures(
                seq, model, learning=learning
            )
            ...
            ReaderWriter.dump_data(
                seg_features, os.path.join(activefeatures_dir, "seg_features")
            )
            # to add condition regarding learning
            ReaderWriter.dump_data(
                l_segfeatures, os.path.join(activefeatures_dir, "l_segfeatures")
            )
    ...
    def get_seq_activatedstates(self, seq_id, seqs_info):
        ...
        activated_states = ReaderWriter.read_data(
            os.path.join(seq_dir, "activated_states")
        )
    ...
    def get_seq_activefeatures(self, seq_id, seqs_info):
        ...
        seq_dir = seqs_info[seq_id]["activefeatures_dir"]
        try:
            activefeatures = ReaderWriter.read_data(
                os.path.join(seq_dir, "activefeatures")
            )
    ...
    def get_imposterseq_globalfeatures(
        self, seq_id, seqs_info, y_imposter, seg_other_symbol=None
    ):
        ...
        l_segfeatures = ReaderWriter.read_data(
            os.path.join(activefeatures_dir, "l_segfeatures"), mode="rb"
        )

        imposter_gfeatures = feature_extractor.extract_seq_features_perboundary(
            seq, l_segfeatures
        )
        ...
        if filter_info["filter_type"] == "count":
            ...
            for z in featuresum_dict:
                for fname, fsum in featuresum_dict[z].items():
                    ...
                    ):  # threshold is of type float -- apply to both categorical and continuous
                        rel_func[relation](fsum, threshold, filtered_dict[z], fname)
