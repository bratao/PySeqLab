...
class AttributeScaler(object):
            ...
            if method == "standardization":
                for attr_name in scaling_info:
                    ...
                    for boundary in boundaries:
                        seg_attr[boundary][attr_name] = (
                            seg_attr[boundary][attr_name] - attr_mean
                        ) / (attr_sd)
                ...
                for attr_name in scaling_info:
                    ...
                    if diff == 0:
                        ...
                        for boundary in boundaries:
                            seg_attr[boundary][attr_name] = self.transform_scale(
                                seg_attr[boundary][attr_name], attr_min, attr_max
                            )

        #                         seg_attr[boundary][attr_name]= (seg_attr[boundary][attr_name] - attr_min)/(diff)
        except Exception as e:
            print(
                "one of the features is either constant or zero. Division by zero error..."
            )
            print(e)
    ...
    def save(self, folder_dir):
        ...
        """
        save_info = {"AS_scalinginfo": self.scaling_info, "AS_method": self.method}
...
class GenericAttributeExtractor(object):
    ...
    def _create_segment(self, X, boundary, attr_names, sep=" "):
        ...
        for attr_name in attr_names:
            ...
            self.seg_attr[boundary][attr_name] = attr_desc[attr_name]["repr_func"](
                segment_value, sep
            )
...
class NERSegmentAttributeExtractor(GenericAttributeExtractor):
    ...
    def generate_attributes_desc(self):
        ...
        attr_desc["shape"] = {
            "description": "the shape of the word",
            "encoding": "categorical",
        }
        attr_desc["shaped"] = {
            "description": "the compressed/degenerated form/shape of the word",
            "encoding": "categorical",
        }
        attr_desc["seg_numchars"] = {
            "description": "number of characters in a segment",
            "encoding": "continuous",
        }
        attr_desc["seg_len"] = {
        ...
        for attr_name in attr_names:
            ...
            for attr_value, count in count_dict.items():
                ...
                if fkey not in attr_desc:
                    attr_desc[fkey] = {
                        "description": "{} -- bag of attributes property".format(
                            "fkey"
                        ),
                        "encoding": "continuous",
                    }
